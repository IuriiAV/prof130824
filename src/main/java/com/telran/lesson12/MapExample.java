 ...
 * Map :
 * Одно значение в мапе єто пара элементов : key -> value
 * Пара неделимая, одному ключу соответствует свое значение
 * Map -> HashMap -> LinkedHashMap
 * Map - > SortedMap -> TreeMap
 * для параллельной работі  -> ConcurrentHashMap
 *
 * put(key, value) - помещает пару в мапу
 * Значения в мапу помещается по ключу:
 * map.put ("apple", 100); - это значит, что по ключу "apple"
 * помещается значение 100
 *
 * Для получения значения по ключу используется метод value = get(key), в параметры метода передается ключ,
 * ответом будет то значение, что есть по ключу
 *
 * Ключ - это уникальное значение, дублей быть не может
 * Ключ в мапе это и есть тот объект, у которого вычисляется hashcode и
 * на основаниии которого исчется место во внутреннем массиве куда положить нашу пару
 *
 ...
 * помещается всегда во внутренний масссив table[0]
 *
 */
